...
scad.cat is a browser-based (and desktop-native) parametric 3D model configurator. The goal is a tight feedback loop: tweak parameters, see the model update in real time, download your STL and go print it.
...
| CAD kernel | OpenSCAD compiled from source — WASM via `openscad-lib/`; desktop via linked library or subprocess |
| Geometry backend | [Manifold](https://github.com/elalish/manifold) (selected at runtime via `--backend Manifold`) |
...
#### WASM — rebuilding the OpenSCAD module (optional)

The compiled `openscad.js` and `openscad.wasm` are committed to the repo so
CI and fresh clones work without Docker. If you need to rebuild them from
source (e.g. after updating the OpenSCAD submodule), you need Docker or Podman:
...
The pre-compiled `openscad.js` + `openscad.wasm` in `web/openscad-wasm-dist/`
are picked up automatically. To rebuild them from source:

```bash
mise run build-openscad   # requires Docker or Podman, takes ~10–20 min
```

This uses the `openscad/wasm-base` Docker image maintained by the OpenSCAD
project, which has all dependencies (Boost, CGAL, GMP, MPFR, Freetype,
HarfBuzz, FontConfig, etc.) pre-built for Emscripten 4.0.10.

---

## How it works

- Select a generator from the sidebar and tweak its parameters
- The model rebuilds automatically ~100 ms after you stop editing
...
- Download the result as a binary STL
...
process, compiled with the Manifold geometry backend.
...
```
src/
  main.cpp              # app loop, UI, AppState, platform glue
  scad_eval.h/cpp       # OpenSCAD evaluation (three backends, one interface)
  viewer.h/cpp          # FBO-based 3D mesh viewer, STL parser, orbit camera
  generator.h/cpp       # generator registry, parameter model
  generator_ui.h/cpp    # ImGui parameter panel
  scad_builder.h/cpp    # generates SCAD source from parameter values
  i18n.h                # .po parser and _() lookup
generators/
  gridfinity_extended/  # bundled generator: Gridfinity Extended
    *.scad              # OpenSCAD library
    *.json              # parameter schema
openscad-lib/
  CMakeLists.txt        # builds OpenSCADLibInternal + openscad_api for desktop
  Dockerfile            # Docker build: openscad/wasm-base + OpenSCAD → WASM
  build-wasm.sh         # runs the Docker build, deposits artifacts
  src/
    openscad_api.h/cpp  # C wrapper: openscad_evaluate() → binary STL bytes
  upstream/
    openscad/           # git submodule: openscad/openscad main branch
models/
  example.scad          # fallback model
external/
...
web/
  ...
  openscad-wasm-dist/   # compiled WASM artifacts (committed to repo)
i18n/
  en.po                 # English (keys only, no translations needed)
  cat.po                # Catalan
```
